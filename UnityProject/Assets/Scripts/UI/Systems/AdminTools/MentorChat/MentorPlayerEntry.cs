...
{
	public class MentorPlayerEntry : MonoBehaviour
	{
		private Action<MentorPlayerEntry> OnClickEvent;
		...
		public MentorPlayerEntryData PlayerData { get; set; }

		public void UpdateButton(MentorPlayerEntryData playerEntryData, Action<MentorPlayerEntry> onClickEvent, GUI_Notification masterNotification = null,
			bool disableInteract = false)
		{
			parentNotification = masterNotification;
			OnClickEvent = onClickEvent;
			PlayerData = playerEntryData;
			displayName.text = $"{playerEntryData.name} - {playerEntryData.currentJob}. ACC: {playerEntryData.accountName} UUID { playerEntryData.uid}";
